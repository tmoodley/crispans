<template>
  <div>
    <b-form-group label="Please Select Your Industries">
  ...
  export default { 
    ...
    methods: {
      ...
      onOptionClick(option) {
        ...
        this.addIndustry(category);
        this.search = '';
      },
      onRemoveClick(tag) { 
        var category = this.categories.filter(x => x.name == tag);
        this.removeIndustry(category);
      },
      getIndustries() {
        var self = this; 
        var cats = [];
        if (self.store.company.customerIndustries != null) {
          cats = self.store.company.customerIndustries.map(x => x.industryId);
        }
        
        return axios
          .get('/portal/api/Industries/')
