<template>
  <div>
    <b-form-group label="Choose Capabilities">
  ...
  export default {
    ...
    methods: {
      ...
      onOptionClick({ option, addTag }) {
        ...
        this.addCapability(category);
        this.search = '';
      },
      onRemoveClick(tag) { 
        var category = this.categories.filter(x => x.name == tag);
        this.removeCapability(category);
      },
      getCapabilities() {
        var self = this; 
        var cats = [];
        if (self.store.company.customerCapabilities != null) {
          cats = self.store.company.customerCapabilities.map(x => x.capabilityId);
        }
        
        return axios
          .get('/portal/api/Capabilities/')
